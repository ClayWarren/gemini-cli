...
import { EditorType } from '@google/gemini-cli-core/src/utils/editor.js';
...
describe('useGeminiStream', () => {
  ...
  beforeEach(() => {
    ...
    _mockCancelAllToolCalls.mockClear();
    mockMarkToolsAsSubmitted.mockClear();
  ...
  const renderTestHook = (initialToolCalls: TrackedToolCall[] = []) => {
    ...
    const { result, rerender } = renderHook(
      ({ _toolCalls }: { _toolCalls: TrackedToolCall[] }) =>
        useGeminiStream(
          clientInstance,
          [],
          mockAddItem,
      ...
      {
        initialProps: { _toolCalls: initialToolCalls },
      },
    );
    return {
      result,
  ...
  it('should submit tool responses when all tool calls are completed and ready', async () => {
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        ...
        response: {
          callId: 'call1',
          responseParts: [{ text: 'tool 1 final response' }],
          error: undefined,
          resultDisplay: 'Tool 1 success display',
        },
      } as TrackedCompletedToolCall,
    ...
    // Directly pass the completed tool calls to renderTestHook
    const { mockMarkToolsAsSubmitted, mockSendMessageStream } =
      renderTestHook(completedToolCalls);
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const { mockMarkToolsAsSubmitted, client } =
      renderTestHook(cancelledToolCalls);
  ...
  it('should not flicker streaming state to Idle between tool completion and submission', async () => {
    ...
    const {
      ...
      rerender,
      client: _client,
    } = renderTestHook(initialToolCalls);
    ...
    act(() => {
      rerender({ _toolCalls: completedToolCalls });
    });
  ...
  describe('Client-Initiated Tool Calls', () => {
    it('should execute a client-initiated tool without sending a response to Gemini', async () => {
      ...
      vi.mocked(useReactToolScheduler).mockReturnValue([
        [],
        mockScheduleToolCalls,
        mockMarkToolsAsSubmitted,
      ]);
